          ...
          renderItem={({ item }) => (
            <View style={styles.productItem}>
              ...
              <TouchableOpacity onPress={() => removeFromCart(item.id)}>
                <Text style={styles.removeLink}>Remover do carrinho</Text>
              </TouchableOpacity>
            </View>
          )}
        />
      )}
      <Button title="Ir para pagamento" onPress={handleCheckout} />
      <TouchableOpacity onPress={() => navigation.navigate('Store')}>
        <Text style={styles.link}>Adicionar mais produtos</Text>
      </TouchableOpacity>
    </View>
  );
};
